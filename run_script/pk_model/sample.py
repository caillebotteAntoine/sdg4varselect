...
from sdg4varselect.pk_model import (
    pk_curve,
    pk_curve_float,
)
...
def nlmem_simulation(params, PRNGKey, N_IND, J_OBS, t_min, t_max):
    ...
    def pk_fct(time, D, phi1, phi2, phi3):
        return pk_curve(time=time, D=D, ka=phi1, Cl=phi2, V=phi3)
...
def cox_weibull_simulation(params, PRNGKey, N_IND, logistic_sim):
    ...
    link_kwargs = {
        "D": -100,
        "ka": logistic_sim["phi1"],
        "Cl": logistic_sim["phi2"],
        "V": params.mu3,
    }

    def link_fct(t, alpha, D, ka, Cl, V):
        return alpha * pk_curve_float(t, D, ka, Cl, V)
