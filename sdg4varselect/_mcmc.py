...
class MCMC(Chain):
    ...
    sample(key, theta_reals1d, size=1, **kwargs) -> list[jnp.ndarray]:
    ...
    def acceptance_rate(self, i: int = None) -> jnp.ndarray:
        """Calculate the acceptance rate of the gibbs sampler iteration.
        ...
        if i is None:
            ...
            rate = jnp.array(self.__acceptance, dtype=jnp.float64) / jnp.array(
    ...
    def sample(
        self, key, theta_reals1d: jnp.ndarray, size=1, **kwargs
    ) -> list[jnp.ndarray]:
    ...
    def gibbs_sampler_step(self, key, theta_reals1d: jnp.ndarray, **kwargs):
