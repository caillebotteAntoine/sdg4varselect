...
def autoproperty(attribute_name):
...
# class is_an_GDres_handler(can_be_lightened, is_iterable, has_last_theta):
#     def __init__(self):
#         super().__init__()
...
class has_chrono:
    def __init__(self, list_with_chrono: list):
        ...
        for i, item in enumerate(list_with_chrono):
...
class MultiRunRes(sdg4vsResults, has_chrono, can_be_lightened, has_last_theta):
    def __init__(self, multi_run: list):
        ...
        self._nrun = len(multi_run)

    multi_run = autoproperty("multi_run")
    nrun = autoproperty("nrun")

    def __len__(self):
        return self.nrun

    def __getitem__(self, i):
        return self._multi_run[i]

    def __iter__(self):
        for res in self._multi_run:
            yield res
...
class RegularizationPathRes(sdg4vsResults, has_chrono, can_be_lightened):
    ...
    def final_result(self):
        return self._multi_run[self._argmin_bic]

    def __len__(self):
        return len(self._multi_run)
...
class MultiRegRes(sdg4vsResults, has_chrono, can_be_lightened, has_last_theta):
...
class TestResults(sdg4vsResults, has_chrono, can_be_lightened, has_last_theta):
    def __init__(self, tests: list[MultiRegRes], test_config: list[dict]):
        ...
        self._tests = tests
        self._ntest = len(tests)
        self._config = test_config

    tests = autoproperty("tests")
    ntest = autoproperty("ntest")
    ...
    def __getitem__(self, i):
        ...
        #     if self._confi[key] == value:
        #         extracted_test

    def __iter__(self):
        for res in self._tests:
            yield res
